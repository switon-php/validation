 ...
 *
 * Road-signs:
 * - validate(): run one constraint against the current field/value
 * - addError(): record one formatted field error
 * - getErrors(): finalize aggregated failures
 *
...
class Validation
{
     ...
     * @param array<string, mixed>|object $source Source data being validated
     */
    ...
    public function validate(ConstraintInterface $constraint): bool
    {
        ...
        if (!$constraint->validate($this) && !isset($this->errors[$this->field])) {
            ...
            $labels = $constraint instanceof LabelAwareConstraintInterface
